...
#dual_y_delta: distance between 2 subsequent y-nodes in the dual grid of H_x and H_y; delta y_j*=(y_j+y_j-1)/2
    ...
    def add_PML(self, N_PML,plot=True):
    ...
    def add_refinement(self, N_original, N_replacement, x_lower,x_upper, plot=True):
        self.n_cells_x = self.n_cells_x + N_original*(N_replacement-1)
        
        i_lower=np.argmin(np.abs(self.mesh.x_vector_ex-x_lower))
        i_upper=np.argmin(np.abs(self.mesh.x_vector_ex-x_upper))
        
        new_steps = np.linspace(x_lower,x_upper,N_original*N_replacement+1)[1:-1]
        
        self.primary_x_vector = np.concatenate((self.primary_x_vector[:i_lower+1],new_steps,self.primary_x_vector[i_upper:]))
        ...
        self.grid_hy=np.meshgrid(self.x_vector_hy,self.y_vector_hy,indexing='ij')
        if plot:
            self.plot_mesh('Plot after addition of cell refinement')
