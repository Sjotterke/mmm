...
class Yee_FDTD:
    ...
    def add_refinement(self,epsilon_r,mu_r,n_cells_y,N_original, N_replacement, x_lower,x_upper,y_lower,y_upper):
        ...
        diff_i = i_upper - i_lower
        
        j_lower=np.argmin(np.abs(self.mesh.y_vector_ex-y_lower))
        j_upper=np.argmin(np.abs(self.mesh.y_vector_ex-y_upper))
        ...
        self.epsilon_r = np.c_[self.epsilon_r[:,:i_lower],self.extra_eps,self.epsilon_r[i_lower+1]]
